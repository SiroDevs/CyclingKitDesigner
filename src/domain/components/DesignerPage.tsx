...
import { designerReducer, initialState } from '@/domain/lib/designerReducer'
import { JERSEY_PARTS, BIBS_PARTS } from '@/domain/lib/constants'
import { PatternType } from '@/domain/lib/types'
import { getPatternDef } from '@/domain/lib/patternUtils'
import { exportSVG, exportAllViews, ExportFormat } from '@/domain/lib/exportUtils'
import Toolbar from '@/domain/components/Toolbar'
import KitCanvas from '@/domain/components/kit/KitCanvas'
import PartsList from '@/domain/components/sidebar/PartsList'
import KitPresets from '@/domain/components/sidebar/KitPresets'
import ColorPanel from '@/domain/components/color/ColorPanel'
import ExportModal from '@/domain/components/ExportModal'
import JerseyFront from '@/domain/components/kit/JerseyFront'
import JerseyBack  from '@/domain/components/kit/JerseyBack'
import BibsFront   from '@/domain/components/kit/BibsFront'
import BibsBack    from '@/domain/components/kit/BibsBack'

export default function DesignerPage() {
  const [state, dispatch] = useReducer(designerReducer, initialState)
  const [exportOpen, setExportOpen] = useState(false)

  // Refs to hidden SVG wrappers used for export
  ...
  // Shared props for hidden export renders (no selection highlight)
  const kitProps = {
    getColor,
    getPatFill,
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8f8f6]">
      ...
      {/* Hidden off-screen renders — always in DOM so refs are ready for export */}
