...
export const COMMON_PATHS = {
  leftSleeve: JERSEY_PATHS.leftSleeve,
  leftCuff: JERSEY_PATHS.leftCuff,
  rightSleeve: JERSEY_PATHS.rightSleeve,
  rightCuff: JERSEY_PATHS.rightCuff,
...
export const Sleeve = ({
  ...
  const sleevePath = isLeft
    ? COMMON_PATHS.leftSleeve
    : COMMON_PATHS.rightSleeve;
  const cuffPath = isLeft ? COMMON_PATHS.leftCuff : COMMON_PATHS.rightCuff;
  const sleeveTransform = isLeft ? "translate(10,30)" : "translate(195, 30)";
  const cuffTransform = isLeft ? "translate(-2.5,84)" : "translate(41.5, 84)";

  return (
    <g transform={sleeveTransform}>
      <path
        d={sleevePath}
        fill={fill}
        ...
        strokeWidth="0.8"
      />
      <g transform={cuffTransform}>
        <path d={cuffPath} fill={lightenDarken(color, -25)} stroke="none" />
      </g>
    </g>
  );
};
...
export const SidePanel = ({
  ...
  const path =
    position === "left"
      ? COMMON_PATHS.leftSidePanel
      : COMMON_PATHS.rightSidePanel;
  return (
    <path d={path} fill={fill} stroke="rgba(0,0,0,0.08)" strokeWidth="0.5" />
  );
};
